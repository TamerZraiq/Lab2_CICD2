...
@pytest.mark.parametrize("invalid_email", ["plainaddress", "test@", "@domain.com"])
...
def test_update_then_404(client):
 ...
 r1 = client.put("/api/users/3", json=user_payload(uid=3, name="Updatedname"))
 assert r1.status_code == 200
 r2 = client.put("/api/users/10", json=user_payload(uid=3, name="Updatedname"))
